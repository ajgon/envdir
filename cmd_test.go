...
func TestCmd_Success(t *testing.T) {
	t.Run("it properly handles input", func(t *testing.T) {
		var (
	...
	t.Run("it shows help", func(t *testing.T) {
		var (
			cmdStdin  bytes.Buffer
			cmdStdout bytes.Buffer
			cmdStderr bytes.Buffer
		...
		if !strings.Contains(output, "Print version info and exit") {
			t.Errorf("expected help message, got output:\n%s", output)
		}
	})

	t.Run("it shows version info", func(t *testing.T) {
		var (
			cmdStdin  bytes.Buffer
			cmdStdout bytes.Buffer
			cmdStderr bytes.Buffer
		...
		if !strings.Contains(output, "envdir version dev, build unavail (unknown)") {
			t.Errorf("expected version message, got output:\n%s", output)
		}
	})
}
